---
title: "Demo"
description: "Image render hook, shortcode + partial for Hyas sites — one page demo"
lead: "Image render hook, shortcode + partial for Hyas sites — one page demo."
date: 2022-09-27T12:59:30+02:00
lastmod: 2022-09-27T12:59:30+02:00
draft: false
images: []
---

## Render hook

### Image in page bundle

```md
![Red buoy 2](mckayla-crump-sp0AwqjBqUw-unsplash.jpg)
```

![Red buoy 2](mckayla-crump-sp0AwqjBqUw-unsplash.jpg)

### Image in assets directory

```md
![Blue kayak on body of water during golden hour](images/mckayla-crump-9uAvlp07eYw-unsplash.jpg)
```

![Blue kayak on body of water during golden hour](images/mckayla-crump-9uAvlp07eYw-unsplash.jpg)

### Remote image

```md
![Alt tag](https://i.giphy.com/media/hIip12KtxrNU5GG0Ii/giphy.gif?cid=790b7611e72a6d08626c8c4ff1410056fbc51e0ed1f2daab&rid=giphy.gif&ct=s "Title tag")
```

Not (yet) supported.

### Base64 encoded image

```md
![alt attribute][src-attribute]

[src-attribute]:data:image/png;base64,
```

Not (yet) supported.

## Shortcode

### Image in page bundle

```md
{{</* lazyimg img="mckayla-crump-sp0AwqjBqUw-unsplash.jpg" alt="Red buoy 2" class="img-fluid mb-4" */>}}
```

{{< lazyimg img="mckayla-crump-sp0AwqjBqUw-unsplash.jpg" alt="Red buoy 2" class="img-fluid mb-4" >}}

### Image in assets directory

```md
{{</* lazyimg img="images/mckayla-crump-9uAvlp07eYw-unsplash.jpg" alt="Blue kayak on body of water during golden hour" */>}}
```

{{< lazyimg img="images/mckayla-crump-9uAvlp07eYw-unsplash.jpg" alt="Blue kayak on body of water during golden hour" >}}

### Remote image

```md
{{</* lazyimg img="https://i.giphy.com/media/hIip12KtxrNU5GG0Ii/giphy.gif?cid=790b7611e72a6d08626c8c4ff1410056fbc51e0ed1f2daab&rid=giphy.gif&ct=s" */>}}
```

Not (yet) supported.

### Figure

Figure with (optional) caption, using renderer `figure`:

```md
{{</* lazyimg img="mckayla-crump-sp0AwqjBqUw-unsplash.jpg" alt="Red buoy 2" caption="Red buoy" renderer="figure" */>}}
```

{{< lazyimg img="mckayla-crump-sp0AwqjBqUw-unsplash.jpg" alt="Red buoy 2" caption="Red buoy" renderer="figure" >}}


Figure with (optional) caption, using renderer `figure-webp`:

```md
{{</* lazyimg img="mckayla-crump-sp0AwqjBqUw-unsplash.jpg" alt="Red buoy 2" caption="Red buoy" renderer="figure-webp" */>}}
```

{{< lazyimg img="mckayla-crump-sp0AwqjBqUw-unsplash.jpg" alt="Red buoy 2" caption="Red buoy" renderer="figure-webp" >}}

## Partial

See [Render hook](/demo/#render-hook) (uses partial).

## Credits

Photos by [McKayla Crump](https://unsplash.com/@funkmastacrump).
